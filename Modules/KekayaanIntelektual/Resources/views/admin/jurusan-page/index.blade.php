                ...
                <div class="container bg-light rounded border pt-3">
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-success" data-bs-toggle="modal"
                            data-bs-target="#tambahJurusanModal"><i class="bi bi-plus-circle me-1"></i>Tambah
                            Jurusan</button>

                        {{-- Modal Tambah Jurusan --}}
                        <div class="modal fade" id="tambahJurusanModal" tabindex="-1" data-bs-backdrop="static"
                            aria-labelledby="tambahJurusanModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <div class="modal-content p-4">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="tambahJurusanModalLabel">Tambah Jurusan</h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form method="post" action="{{ url('/admin/jurusan') }}" class="mt-3">
                                            @csrf
                                            ...
                                            <div class="form-outline form-white mb-4">
                                                ...
                                                <input type="text" id="kode_jurusan"
                                                    ...
                                                    name="kode_jurusan" value="{{ old('kode_jurusan') }}" />
                                                @error('kode_jurusan')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                            data-bs-target="#editJurusanModal{{ $j->id }}">
                                            <i class="bi bi-pencil"></i>
                                        </button>

                                        {{-- Modal Edit Jurusan --}}
                                        <div class="modal fade" id="editJurusanModal{{ $j->id }}" tabindex="-1"
                                            data-bs-backdrop="static" aria-labelledby="editJurusanModalLabel"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-scrollable">
                                                <div class="modal-content p-2">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5" id="editJurusanModalLabel">Edit
                                                            Jurusan {{ $j->nama_jurusan }}</h1>
                                                        <button type="button" class="btn-close"
                                                            data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form method="post"
                                                            action="{{ url('/admin/jurusan/' . $j->id) }}">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="form-outline form-white mb-3">
                                                                <label class="form-label"
                                                                    for="nama_jurusan_{{ $j->id }}">Nama
                                                                    Jurusan</label>
                                                                <input type="text" id="nama_jurusan_{{ $j->id }}"
                                                                    class="form-control" name="nama_jurusan"
                                                                    value="{{ $j->nama_jurusan }}" required>
                                                            </div>
                                                            <div class="form-outline form-white mb-3">
                                                                <label class="form-label"
                                                                    for="kode_jurusan_{{ $j->id }}">Kode
                                                                    Jurusan</label>
                                                                <input type="text" id="kode_jurusan_{{ $j->id }}"
                                                                    class="form-control" name="kode_jurusan"
                                                                    value="{{ $j->kode_jurusan }}" required>
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="button"
                                        ...
                                        <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                            data-bs-target="#hapusJurusanModal{{ $j->id }}">
                                            <i class="bi bi-trash3"></i>
                                        </button>

                                        {{-- Modal Hapus Jurusan --}}
                                        <div class="modal fade" id="hapusJurusanModal{{ $j->id }}"
                                            data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                                            aria-labelledby="hapusJurusanModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5" id="hapusJurusanModalLabel">
                                                            Peringatan</h1>
                                                        <button type="button" class="btn-close"
                                                            data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        Anda yakin akan menghapus jurusan
                                                        {{ $j->nama_jurusan }} ?
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-outline-secondary"
