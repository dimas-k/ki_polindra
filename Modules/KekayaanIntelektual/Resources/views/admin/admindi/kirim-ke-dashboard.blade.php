...
<head>
    ...
    <title>SIKI POLINDRA-Admin | Desain Industri | Kirim ke Dashboard</title>
    ...
    <div class="container mt-4">
        ...
        @if ($desainIndustri->dikirim_ke)
            <div class="alert alert-warning">
                Data ini sudah pernah dikirim sebagai <strong>{{ ucfirst($desainIndustri->dikirim_ke) }}</strong>.
            </div>
        @else
            <form action="{{ route('admin_desainindustri.kirim', $desainIndustri->id) }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Judul</label>
                    <input type="text" class="form-control" value="{{ $desainIndustri->judul_di }}" disabled>
                </div>
                <div class="mb-3">
                ...
                <a href="{{ route('admin_desainindustri.show', $desainIndustri->id) }}" class="btn btn-secondary">Batal</a>
            </form>
        @endif
    </div>
