<table class="table table-hover font-family-Kokoro">
    ...
    <tbody>
        @foreach ($dosen as $i => $a)
            <tr>
                <th scope="row">{{ $dosen->firstItem() + $i }}</th>
                <td>{{ $a->nama_lengkap }}</td>
                <td>{{ $a->email }}</td>
                <td>{{ $a->nip }}</td>
                <td>{{ $a->jabatan }}</td>
                <td>{{ $a->no_telepon }}</td>
                <td><a href={{ Route('detail.Dosen', $a->id) }} class="btn btn-primary"><i class="bi bi-eye"></i></a>
                        ...
                        <div class="modal-dialog modal-dialog-scrollable">
                            <div class="modal-content p-4">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('dosen.edit', $a->id) }}" enctype="multipart/form-data"
                                        method="POST">
                                        @csrf
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-2">
                                            ...
                                            <p class="text-danger"><i
                                                    class="bi bi-exclamation-triangle-fill"></i>Harap isi password
                                                dengan benar dan atau masukkan password baru</p>
                                            @error('password')
                                                <div class="invalid-feedback">
                                                    {{ $message }}
                                                </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-outline-secondary"
                                        data-bs-dismiss="modal">Batal</button>
                                    <a href={{ Route('dosen.hapus', $a->id) }} class="btn btn-danger">Hapus</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
@if ($dosen->isEmpty())
    <p class="text-center text-muted py-3">Data dosen tidak ditemukan.</p>
@endif
